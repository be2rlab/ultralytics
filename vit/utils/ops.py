...
class HungarianMatcher(nn.Module):
    ...
    def forward(
        ...
        return [
            (
                ...
                torch.tensor(j, dtype=torch.int32) + gt_groups[k],
            )
    ...
    def _cost_mask(self, bs, num_gts, masks=None, gt_mask=None):
        ...
        sample_points = torch.cat(
            [a.repeat(b, 1, 1, 1) for a, b in zip(sample_points, num_gts) if b > 0]
        )
        tgt_mask = F.grid_sample(tgt_mask, sample_points, align_corners=False).squeeze(
            [1, 2]
        )
...
def get_cdn_group(
    ...
    return padding_cls.to(class_embed.device), padding_bbox.to(
        class_embed.device
