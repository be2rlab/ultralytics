...
class DETRLoss(nn.Module):
    ...
    def _get_loss_mask(self, masks, gt_mask, match_indices, postfix=""):
        ...
        if sum(len(a) for a in gt_mask) == 0:
            loss[name_mask] = torch.tensor(0.0, device=self.device)
            loss[name_dice] = torch.tensor(0.0, device=self.device)
            return loss

        num_gts = len(gt_mask)
        src_masks, target_masks = self._get_assigned_bboxes(
        ...
        loss[name_mask] = self.loss_gain["mask"] * F.sigmoid_focal_loss(
            src_masks, target_masks, torch.tensor([num_gts], dtype=torch.float32)
        )
        loss[name_dice] = self.loss_gain["dice"] * self._dice_loss(
    ...
    def _get_assigned_bboxes(self, pred_bboxes, gt_bboxes, match_indices):
        pred_assigned = torch.cat(
            [
                ...
                for t, (I, _) in zip(pred_bboxes, match_indices)  # noqa: E741
            ]
        )
