...
def benchmark(
    ...
    for i, (
        ...
        try:
            ...
            if not (ROOT / "assets/bus.jpg").exists():
                download(
                    url="https://ultralytics.com/images/bus.jpg", dir=ROOT / "assets"
                )
            export.predict(
                ROOT / "assets/bus.jpg", imgsz=imgsz, device=device, half=half
            )
    ...
    if hard_fail and isinstance(hard_fail, float):
        ...
        ), f"HARD FAIL: one or more metric(s) < floor {floor}"
...
class ProfileModels:
    ...
    def profile_tensorrt_model(self, engine_file: str):
        ...
        num_runs = max(
            round(self.min_time / elapsed * self.num_warmup_runs),
    ...
    def profile_onnx_model(self, onnx_file: str):
        ...
        # Compute number of runs as higher of min_time or num_timed_runs
        num_runs = max(
            round(self.min_time / elapsed * self.num_warmup_runs), self.num_timed_runs
        )
