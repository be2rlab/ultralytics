...
class HUBDatasetStats:
    ...
    def _unzip(self, path):
        ...
        return True, str(unzip_dir), self._find_yaml(
            unzip_dir
    ...
    def get_json(self, save=False, verbose=False):
        ...
        def _round(labels):
            ...
            return [
                [int(c), *(round(float(x), 4) for x in points)] for c, points in zipped
            ]
        ...
        for split in "train", "val", "test":
            ...
            x = np.array(
                [
                    np.bincount(
                        label["cls"].astype(int).flatten(), minlength=self.data["nc"]
                    )
                    for label in tqdm(
